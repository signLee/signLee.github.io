function getEle(str,aParent){
	...
	var aParent=aParent||[document];//如果有就用aParent 没有用就docuemnt
	var aChild=[];	//存返回来的子
	
	for(var i=0;i<arr.length;i++){
		aChild = getByStr(aParent,arr[i]);
		aParent = aChild;//返回的子替换成父
	}
...
function getByStr(aParent,str){
	...
	for(var j=0;j<aParent.length;j++){
		...
		switch(str.charAt(0)){//子有几种情况id class tagname
			case '#'://id	==	#div1
				var obj = document.getElementById(str.substring(1));//去除#号
				obj && aChild.push(obj);
				break;
			case '.'://class
				var aEle=getByClass(aParent[j],str.substring(1));//去除.号
			...
			default://tagname
				if(/\w+#\w+/.test(str)){//div#div1
					...
					var aEle=aParent[j].getElementsByTagName(arr[0]);//aDiv先抓取所有的父，再看父元素里有多少符合要求的
					for(var i=0;i<aEle.length;i++){
						if(aEle[i].id==arr[1]){
							aChild.push(aEle[i]);
						}
...
function getByClass(oParent,sClass){
	if(oParent.getElementsByClassName){//优先使用原生方法
...
function ready(fn){
	if(document.addEventListener){
		document.addEventListener('DOMContentLoaded',fn,false);	//for高版本  DOM加载完毕
	}else{
		document.attachEvent('onreadystatechange',function(){//for IE chrom
...
function getStyle(obj,attr){//获取行间/非行间样式
	return (obj.currentStyle||getComputedStyle(obj,false))[attr] //false 也可以换成伪类':sdfsf'
}
...
function move(obj,json,opational){
	...
	var dis={};
	for(var key in json){
		start[key]=parseFloat(getStyle(obj,key));//解决透明度为小数的问题
		dis[key]=parseFloat(json[key])-start[key];
	}
	
	var count=Math.round(opational.time/30);//防止除不尽定时器一直走的问题
	var n=0;
...
function ajax(options){
	...
	options.timeout=options.timeout||0;//默认不做超时响应
	options.success=options.success||null;
	options.error=options.error||null;
	
	
	options.data.t=Math.random();//保证每次的地址不一样
	
	//0.整理接口
	var arr=[];
	for(var key in options.data){  
		arr.push(key+'='+encodeURIComponent(options.data[key]))	//解决输入中文乱码的问题
	}
	var str=arr.join('&');//http://www.baidu.com?key=value&key=value
	
	
	//1创建ajax对象
	if(window.XMLHttpRequest){//for 高版本
	...
	switch(typeof args){
		...
		case 'object':
			if('length' in args){//判断传过来的对象是一个还是多个
				this.elements=args;//包装this.elements	
			}else{
...
zQuery.prototype.css=function(ar1,ar2){
	if(arguments.length==2){//2   XXX.css(width,'200px');
		//修改
		...
		}else if(typeof ar1=='object'){//批量设置   XXX.css({width:'200px',height:'200px'})
			for(var i=0;i<this.elements.length;i++){
				for(var key in ar1){
...
'click/mouseover/mouseout/contextmenu'.replace(/\w+/g,function(sEvt){//sEvt----符合规则的字符 click  mouseover ……
...
zQuery.prototype.toggle=function(){
	...
	for(var i=0;i<this.elements.length;i++){
		...
		addEvent(this.elements[i],'click',function(ev){
			if(args[this.count%args.length].call(this,ev)==false){//看是否需要阻止默认
				this.count++;
				return false;	
			}else{
				this.count++;	
...
zQuery.prototype.index=function(){
	...
	for(var i=0;i<aChild.length;i++){//遍历判断每个子是不是
		if(aChild[i]==this.elements[this.elements.length-1]){ //jq里如果找一堆元素的index默认找的是最后一个的索引
			return i;
		}
	}
};
...
zQuery.prototype.removeClass=function(sClass){
	...
	for(var i=0;i<this.elements.length;i++){
		...
		if(!this.elements[i].className){
			this.elements[i].removeAttribute('class');	//去掉页面上的class
		}
	}
...
zQuery.prototype.toggleClass=function(sClass){//切换class
	for(var i=0;i<this.elements.length;i++){
...
zQuery.prototype.each=function(fn){//循环
	for(var i=0;i<this.elements.length;i++){
		fn.call(this.elements[i],i,this.elements[i]);	
	}
};
$.fn=zQuery.prototype;	//把往ZQ里定义的插件引用指向原型  $.fn.aaa=function(){alert('XXX')};
zQuery.prototype.extend=function(json){//$().extend({key:fn})   批量定义插件
	for(var key in json){
